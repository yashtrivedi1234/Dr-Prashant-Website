...
export const verifyEmailService = async () => {
  console.log('🔍 Checking Resend Email Service...');
  ...
  try {
    // Resend is a simple API client, so we just verify the API key is set
    if (!process.env.RESEND_API_KEY || !process.env.FROM_EMAIL) {
      throw new Error('Missing required environment variables (RESEND_API_KEY or FROM_EMAIL)');
    }
    ...
    console.log(`   - Verify RESEND_API_KEY is set in environment variables`);
    console.log(`   - Ensure FROM_EMAIL is configured in .env\n`);
    return false;
  }
};
